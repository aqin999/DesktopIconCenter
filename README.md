...
从 v1.0.4 开始，检测到 360 桌面助手的 `360DirectUICls` / `360DTFenceLite` 覆盖桌面时，程序会优先使用真实鼠标输入拖拽当前 360 桌面图标。该路径不需要重载 360 布局，成功后会等待 `DTFenceData.dtf` 中目标图标 order 同步变化，再写入“移动成功”日志。

为避免误拖其它窗口，真实拖拽只会在源位置和目标位置当前都命中 360 桌面窗口时执行；如果桌面被浏览器、资源管理器或其它窗口遮挡，程序会放弃真实拖拽并自动回退到 v1.0.3 的稳定兜底路径：修改 `DTFenceData.dtf` 后临时重启 360 桌面助手加载布局。
