...
从 v1.0.3 开始，检测到 360 桌面助手的 `360DirectUICls` / `360DTFenceLite` 覆盖桌面时，程序不再把“发送拖拽消息”或“写入 `DTFenceData.dtf`”直接当成成功。

当前可复现生效路径是：等待 360 写入新图标记录 → 临时停止 360 桌面助手进程 → 修改 `DTFenceData.dtf` 中目标图标 order → 重新启动 360 桌面助手加载布局。这样日志中的“移动成功”对应的是 360 重新加载后的可见位置，而不是仅修改文件。
